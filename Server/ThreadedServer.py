                    ...
                    if data == "Connecting2RGBWebcam":
                        ...
                        #self.ret = False;
                            ...
                            if frame is not None:
                                ...
                                gzip_cpr_data = z.compress(np.asarray(frame)) + z.flush()
                                client.send(gzip_cpr_data)                                
...
#                            client.send("new body frame")
                        ...
                        if self._kinect.has_new_depth_frame():
                            ...
                            #np.savez_compressed(compressed_array,np.asarray(f))
                            #client.send(np.asarray(f))
                            z = zlib.compressobj(-1,zlib.DEFLATED,31)
                            gzip_cpr_data = z.compress(np.asarray(f)) + z.flush()
                            client.send(gzip_cpr_data)
                        else:
